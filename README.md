...
`VITE_IAG_WRITE_TOKEN` is suitable only for a trusted local development session because Vite variables are visible to the browser; use an authenticated gateway or identity provider for shared/production deployments.
