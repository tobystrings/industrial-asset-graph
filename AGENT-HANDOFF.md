...
**September 10 appearance:** The owner requested black-and-white dark mode throughout the application. Keep neutral charcoal surfaces and light text in all workspaces, authentication, Map Studio, and standalone viewer/tour controls. `src/ui/dark-theme.css` defines native dark controls and display-only inversion for map/cabinet drawings; preserve original evidence files and saved symbol colors. It loads immediately before the final chrome-clearance stylesheet. The visual audit checks dark neutral shell surfaces and measured text contrast in addition to all existing geometry and workflow checks.
