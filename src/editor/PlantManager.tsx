...
function DataHealthPanel() {
  ...
  const focusTarget = (detail: { assetId?: string; areaId?: string; relationship?: boolean }) => {
    if (detail.areaId) { navigate('area', {area:detail.areaId,tab:'record'}); return; }
    const asset = facility.assets.find(item => item.id === detail.assetId)?.id
      ?? facility.components.find(item => item.id === detail.assetId)?.parentId;
    navigate(detail.relationship ? 'relationships' : 'asset', {
      ...(asset ? {asset} : {}), ...(detail.relationship ? {trace:'direct'} : {tab:'record'}),
    });
  };
  return <div className="iag-editor-form iag-health-panel">
    ...
    <section className="iag-pending-changes"><div className="iag-section-head"><strong>Next verification targets</strong><span>{unknowns.length + verifyRelationships.length + disputedRelationships.length + lowCoverageAreas.length}</span></div>{lowCoverageAreas.map(({ area, assets }) => <button className="iag-target-row" type="button" key={`area:${area.id}`} onClick={() => focusTarget({ areaId: area.id })}><strong>{area.name}</strong><small>{assets.length ? `${assets.length} documented asset(s); review FIELD_VERIFY records` : 'No documented assets; start field verification'}</small><b>Open →</b></button>)}{unknowns.map(({ asset, unknown }) => <button className="iag-target-row" type="button" key={`${asset.id}:${unknown}`} onClick={() => focusTarget({ assetId: asset.id })}><strong>{asset.id}</strong><small>{unknown}</small><b>Open →</b></button>)}{verifyRelationships.map((relationship) => <button className="iag-target-row" type="button" key={relationship.id} onClick={() => focusTarget({ assetId: relationship.source, relationship: true })}><strong>{relationship.source} → {relationship.target}</strong><small>{relationship.type} · FIELD_VERIFY · {relationship.evidenceIds.length} evidence reference(s)</small><b>Trace →</b></button>)}{disputedRelationships.map((relationship) => <button className="iag-target-row" type="button" key={relationship.id} onClick={() => focusTarget({ assetId: relationship.source, relationship: true })}><strong>{relationship.source} → {relationship.target}</strong><small>{relationship.type} · DISPUTED · review evidence before promotion</small><b>Trace →</b></button>)}</section>
  </div>;
}
