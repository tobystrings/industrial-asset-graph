...
export default function Troubleshooting({assetId}:{assetId:string}){
 ...
 return <section className="troubleshooting" data-testid="troubleshooting-workspace">
  ...
  {!session?<>
   ...
   <label>What stopped working?<select aria-label="What stopped working?" value={symptom} onChange={e=>setSymptom(e.target.value)}>{symptoms.map(s=><option key={s.id} value={s.id}>{s.label}</option>)}</select></label>
   {Object.entries(intakeFields).map(([key,label])=><label key={key}>{label}{key==='role'?<select aria-label={label} value={intake.role} onChange={e=>setIntake({...intake,role:e.target.value})}><option value="operator">Line operator</option><option value="maintenance">Maintenance technician</option><option value="relief">Shift relief</option><option value="Unknown">Unknown</option></select>:<input value={intake[key]??''} placeholder="Unknown" onChange={e=>setIntake({...intake,[key]:e.target.value})}/>}</label>)}
   ...
   {panel==='checks'&&<>
    ...
    {check?<article className="slate-card"><h3>{check.title}</h3><p><strong>Observe:</strong> {check.observe}</p><p><strong>Where:</strong> Normal operating position; exact display and sensor locations are unverified.</p><p><strong>Normal:</strong> {check.normal}</p><p><strong>Who:</strong> Operators may observe from outside guards. Anything requiring access routes to qualified maintenance.</p>
     ...
     <details><summary>What each answer means</summary><p>Yes: {check.yes}</p><p>No: {check.no}</p><p>Not sure: {check.unknown}</p></details>
   ...
   {panel==='recovery'&&<>
    <h3>Recovery is a separate decision</h3><label>Action being considered<select aria-label="Action being considered" value={draft.recoveryKind} onChange={e=>patch({recoveryKind:e.target.value})}>{Object.entries(recoveryKinds).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></label>
   ...
   {panel==='outcome'&&<>
    ...
    <label>Outcome<select aria-label="Outcome" value={draft.outcome} onChange={e=>patch({outcome:e.target.value as Session['payload']['outcome']})}><option value="unresolved">Issue remains unresolved</option><option value="escalated">Maintenance escalation required</option><option value="restored">Operation restored and verified</option></select></label>
    {draft.outcome==='restored'&&Object.entries(restartFields).map(([k,v])=><label key={k}>{v}<select aria-label={v} value={draft.restart[k]??'unknown'} onChange={e=>patch({restart:{...draft.restart,[k]:e.target.value}})}>{Object.entries(labelAnswer).map(([a,l])=><option key={a} value={a}>{l}</option>)}</select></label>)}
