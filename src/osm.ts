export interface BuildingFootprint { id: string; name: string; heightMeters: number; approximateHeight: boolean; sourceLabel: string; recordId: string; sourceUrl: string; capturedAt: string; geometry: [number, number][]; }
...
async function loadPortlandBuildings(latitude: number, longitude: number, radius: number): Promise<BuildingFootprint[]> {
  ...
  return (data.features ?? []).flatMap((feature, index) => {
    ...
    return [{ id: `portland-${recordId}`, name, heightMeters, approximateHeight: !(Number.isFinite(feet) && feet > 0), sourceLabel: 'Portland public building footprint', recordId, sourceUrl, capturedAt, geometry }];
  });
}
...
export async function loadOpenStreetMapContext(latitude: number, longitude: number, radius = 400): Promise<OpenStreetMapContext> {
  ...
  if (portlandBuildings.length) return { buildings: portlandBuildings, streets: portlandStreets, buildingSource: 'Portland public GIS', streetSource: portlandStreets.length ? 'Portland public GIS' : 'None' };
  ...
  try {
    ...
    const buildings = ways.filter((way) => way.tags?.building).slice(0, 350).flatMap((way) => {
      ...
      return [{ id: `osm-${way.id}`, name: way.tags?.name ?? way.tags?.building ?? `OSM building ${way.id}`, heightMeters, approximateHeight, sourceLabel: 'OpenStreetMap building', recordId: String(way.id), sourceUrl: `https://www.openstreetmap.org/way/${way.id}`, capturedAt: new Date().toISOString(), geometry }];
    });
    ...
    if (portlandBuildings.length) return { buildings: portlandBuildings, streets: contextStreets, buildingSource: 'Portland public GIS', streetSource };
    if (ways.length) return { buildings, streets: contextStreets, buildingSource: 'OpenStreetMap', streetSource };
