...
export default function FieldDocumentationWorkspace({ selectedAsset, onAsset, onChanged, onExit }: { selectedAsset: FacilityAsset | null; onAsset: (asset: FacilityAsset) => void; onChanged: () => void; onExit: () => void }) {
  ...
  const [section, setSection] = useState<'checklist' | 'connections' | 'evidence'>('checklist');
  ...
  return <section className="field-doc-workspace panel" data-testid="field-documentation-mode" data-section={section}>
    ...
    <div className="field-doc-toolbar">
      <details className="field-initials"><summary>Capture initials · {technician}</summary><label>Technician<input value={technician} onChange={(event) => setTechnician(event.target.value)} placeholder="Initials or name" /></label><button type="button" onClick={identify}>Use these initials</button></details>
    ...
    {!selectedAsset ? <div className="field-doc-empty"><b>Select the asset physically in front of you.</b><span>Field notes remain local and reviewable; they do not silently become verified plant facts.</span></div> : <>
      ...
      <nav className="field-section-tabs" aria-label="Field sections">{(['checklist','connections','evidence'] as const).map(id => <button key={id} type="button" aria-pressed={section === id} onClick={() => setSection(id)}>{id === 'checklist' ? 'Checklist' : id === 'connections' ? 'Connections' : 'Evidence'}</button>)}</nav>
