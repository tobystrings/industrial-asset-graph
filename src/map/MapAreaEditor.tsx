...
export function useMapEditorSession(active: boolean, onExit: () => void) {
  ...
  const cancel = () => { if(saving) return; if(dirty&&!window.confirm('Discard unsaved Map Studio edits?')) return; void clearSavedDraft(); setHistory(createHistory(baseline.current)); setAssets(structuredClone(facility.assets)); setSelection([]); setWorkingPoints([]); onExit(); };
...
export function MapEditorToolbar({ session }: { session: MapEditorSession }) {
  ...
  return <div className="map-editor-shell" aria-label="Map and area editor">
    ...
    {panel && <div id="map-studio-panel" className="map-tool-panel">
    ...
    <div className="map-editor-message" data-initial={!session.errors.length && session.message==='Select an editing tool. Structural geometry and manual markup are separate layers.'} role="status">{session.errors[0] ?? session.message}</div>
...
export function MapEditorSvgLayer({ session, width, height }: { session: MapEditorSession; width: number; height: number }) {
  ...
  return <svg className={`map-editor-svg tool-${session.tool} ${session.previewDraft ? 'studio-preview' : ''}` } viewBox={`0 0 ${width} ${height}`} aria-label="Editable facility geometry" onPointerDown={session.pointerDown} onPointerMove={session.pointerMove} onPointerUp={session.pointerUp} onPointerCancel={session.pointerUp}>
    ...
    {layerState(draft.mapConfig,'equipment').visible && <g className="studio-equipment">{draft.mapConfig.markers?.map(m=><g key={m.id} className={`placement-${m.tone}`} role="button" aria-label={`Edit placement ${m.label}`} onPointerDown={event=>session.startDrag(event,{kind:'marker',id:m.id})}><title>{m.label}</title><circle cx={m.x/100*width} cy={m.y/100*height} r={22} className="placement-hit"/><circle cx={m.x/100*width} cy={m.y/100*height} r={9} className={session.selection.some(s=>s.id===m.id)?'selected':''}/><text x={m.x/100*width+12} y={m.y/100*height+4}>{m.id}</text></g>)}</g>}
