...
export function useMapEditorSession(active: boolean, onExit: () => void) {
  ...
  const save = async () => {
    ...
    try { await editor.saveMapDraft({ ...history.present, assets }, summary); baseline.current = structuredClone(history.present); setHistory(createHistory(history.present)); setMessage(`Saved ${summary.length} structural or markup change${summary.length === 1 ? '' : 's'}.`); }
    catch (error) { setMessage(error instanceof Error ? error.message : 'Map changes could not be saved.'); }
  };
