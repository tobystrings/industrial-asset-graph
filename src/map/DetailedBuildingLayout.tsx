import { AreaMapLabel } from './AreaMapLabel';
import { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { useFacility } from '../facility';
import type { FacilityMapAnnotation, FacilityMapMarker, FacilityMapWall } from '../facility/types';
import type { FacilityArea, FacilityAsset, VerificationState } from '../types/facility';
import buildingLayoutImage, { referenceLabelMasks } from './embeddedBuildingLayoutImage';
import './detailedBuildingLayout.css';
import { areaColor } from './mapPalette';
import { MapStudioPanel } from './MapStudioPanel';
import { ReferenceCleanup, SavedStudioSymbols } from './StudioSymbols';
...
import { MapEditorSvgLayer, MapEditorToolbar, useMapEditorSession } from './MapAreaEditor';
...
export default function DetailedBuildingLayout({ selectedArea, selectedAsset, filters, traceAssetIds, onArea, onAsset }: Props) {
  ...
  const walls = (mapConfig?.walls ?? []) as FacilityMapWall[];
  const annotations = (mapConfig?.annotations ?? []) as FacilityMapAnnotation[];
  ...
  const leaveMapEditor = useCallback(() => { setEditMode(false); window.dispatchEvent(new CustomEvent('iag-map-edit-mode', { detail: false })); }, []);
  const mapEditor = useMapEditorSession(editMode, leaveMapEditor);
  const displayConfig = editMode ? (mapEditor.previewDraft ?? mapEditor.history.present).mapConfig : mapConfig ?? {};
  ...
  const legendContent = <div className="legend-grid"><span><i className="swatch outline cabinet" />Control cabinet</span><span><i className="swatch outline machine" />Machine / equipment</span><span><i className="swatch outline power" />Power distribution</span><span><i className="swatch selection" />Selected object</span></div>;
  const displayAreas = editMode ? (mapEditor.previewDraft ?? mapEditor.history.present).areas : areas;
  ...
  return <section className={`reference-layout ${editMode ? 'map-studio' : ''} studio-pane-${studioPane}`} aria-label="Building Layout">
    ...
    <aside className="map-readable-legend" aria-label="Map color legend">
      <button type="button" aria-expanded={legendOpen} onClick={()=>setLegendOpen(!legendOpen)}>Map legend {legendOpen ? '−' : '+'}</button>
      <span>Colors identify areas and equipment. They do not indicate safety or verification.</span>
      {legendOpen && <div className="map-legend-content">{legendContent}<details className="map-area-key-details"><summary>Area colors</summary><div className="map-area-key">{displayAreas.filter(a=>a.visible!==false).map(a=><span key={a.id}><i style={{background:areaColor(a.id)}}/>{a.shortName || a.name}</span>)}</div></details></div>}
    </aside>
    ...
    <div className={`reference-drawing-sheet ${editMode ? 'is-editing' : ''}`}>
      <div ref={planWrapRef} className={`reference-plan-wrap ${mapEditor.tool === 'pan' && editMode ? 'cursor-pan' : ''} ${gesturing ? 'is-panning' : ''}`} tabIndex={0} onKeyDown={handleKeyDown} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} onWheel={wheelZoom}>
        <div className={`reference-plan-transform ${gesturing ? 'is-gesturing' : ''}`} style={{ transform: `translate3d(${view.x}px,${view.y}px,0) scale(${view.scale})` }}>
          <div className="reference-plan-image-stage" style={{ '--facility-drawing-width': `${drawingWidth}px`, '--facility-drawing-ratio': `${drawingWidth} / ${drawingHeight}` } as React.CSSProperties}>
            ...
            <image className="map-reference-artwork" opacity={layerState(displayConfig,'reference').visible ? displayConfig.studio?.referenceOpacity ?? 1 : 0} href={buildingLayoutImage} width={drawingWidth} height={drawingHeight} preserveAspectRatio="xMinYMin meet" onLoad={(event) => { const image = event.currentTarget as SVGImageElement; const box = image.getBBox(); if (box.width && box.height) setNaturalSize({ width: box.width, height: box.height }); }}/>
            <g className="map-reference-label-cleanup" aria-hidden="true" pointerEvents="none">{referenceLabelMasks.map(([x,y,width,height], i) => <rect key={i} x={x / 1536 * drawingWidth} y={y / 1024 * drawingHeight} width={width / 1536 * drawingWidth} height={height / 1024 * drawingHeight} fill="#fff" rx="3"/>)}</g>
            ...
            <g className="svg-zone-layer" style={{display:editMode||!layerState(displayConfig,'areas').visible?'none':undefined}}>{areas.filter(area => area.visible !== false).map((area) => area.overlay ? <g key={area.id} role="button" tabIndex={0} style={{'--area-color':areaColor(area.id)} as React.CSSProperties} className={`svg-zone ${selectedArea?.id === area.id ? 'selected' : ''}`} aria-label={`Select ${area.name}`} onClick={(event) => { event.stopPropagation(); if (!didPan.current) onArea(area); }} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onArea(area); } }}>{area.overlay.polygon ? <polygon points={zoneShape(area)}/> : <rect x={area.overlay.x / 100 * drawingWidth} y={area.overlay.y / 100 * drawingHeight} width={area.overlay.width / 100 * drawingWidth} height={area.overlay.height / 100 * drawingHeight}/>}<AreaMapLabel area={area} width={drawingWidth} height={drawingHeight}/></g> : null)}</g>
            {!editMode && layerState(displayConfig,'walls').visible && <g className="map-editor-walls saved-map-layer" aria-label="Saved structural walls">{walls.map((wall) => <polyline key={wall.id} points={mapPoints(wall.points)}/>)}</g>}
            {!editMode && layerState(displayConfig,'markup').visible && <g className="map-editor-annotations saved-map-layer" aria-label="Saved manual annotations">{annotations.map((annotation) => annotation.kind === 'TEXT' || annotation.kind === 'NOTE' ? <text key={annotation.id} x={annotation.x / 100 * drawingWidth} y={annotation.y / 100 * drawingHeight} fill={annotation.color}>{annotation.text}</text> : annotation.kind === 'RECTANGLE' || annotation.kind === 'CIRCLE' ? <rect key={annotation.id} x={annotation.x / 100 * drawingWidth} y={annotation.y / 100 * drawingHeight} width={annotation.width / 100 * drawingWidth} height={annotation.height / 100 * drawingHeight} stroke={annotation.color}/> : <polyline key={annotation.id} points={mapPoints('points' in annotation ? annotation.points : [])} stroke={annotation.color} className={annotation.kind.toLowerCase()}/>)}</g>}
            {markers.map((marker) => {
              const asset = liveAsset(marker.assetId);
              ...
              const visible = layerState(displayConfig,'equipment').visible && categoryVisible && (marker.state !== 'REFERENCE' || layers.reference) && (marker.state === 'REFERENCE' || !asset || filters.has(asset.verificationStatus));
            ...
            {!editMode&&<svg className="studio-saved-overlay" viewBox={`0 0 ${drawingWidth} ${drawingHeight}`}><SavedStudioSymbols config={displayConfig} width={drawingWidth} height={drawingHeight}/></svg>}
            <MapEditorSvgLayer session={mapEditor} width={drawingWidth} height={drawingHeight}/>
          </div>
        </div>
      ...
      <div className="reference-info-strip"><section><h3>Control Cabinets</h3>{cabinetDirectory}</section><section><h3>Major Machines / Equipment</h3>{machineDirectory}</section><section><h3>Areas</h3>{areaDirectory}</section><section><h3>Notes</h3>{notes}</section></div>
