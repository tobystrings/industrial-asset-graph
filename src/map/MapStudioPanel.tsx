...
export function MapStudioPanel({session:s}:{session:MapEditorSession}) {
  ...
  return <aside className="map-studio-panel" aria-label="Map Studio assistant and properties">
    ...
    {tab==='Objects'&&<section className="studio-tab-body" aria-label="Studio objects">
      ...
      {studio.templates?.length ? <label>Custom symbols<select value={s.template?.id??''} onChange={e=>{s.setTemplate(studio.templates!.find(t=>t.id===e.target.value)??null);s.setSymbolKind('custom');}}><option value="">Choose a template</option>{studio.templates.map(t=><option key={t.id} value={t.id}>{t.label}</option>)}</select></label>:null}
      ...
      {symbol&&<><label>Symbol name<input value={symbol.label} onChange={e=>patchSymbol({label:e.target.value})}/></label><label>Symbol type<select value={symbol.kind} onChange={e=>patchSymbol({kind:e.target.value as FacilityMapSymbol['kind']})}>{symbolKinds.map(k=><option key={k}>{k}</option>)}</select></label><label>Rotation (degrees)<input type="number" value={symbol.rotation} onChange={e=>patchSymbol({rotation:Number(e.target.value)})}/></label><button onClick={()=>patchSymbol({flipped:!symbol.flipped})}>Flip swing / orientation</button>{symbol.kind==='stairs'&&<><label>Step count<input type="number" min="2" max="30" value={symbol.steps??7} onChange={e=>patchSymbol({steps:Number(e.target.value)})}/></label><label>Stair direction<select value={symbol.direction??'up'} onChange={e=>patchSymbol({direction:e.target.value as 'up'|'down'})}><option value="up">Up</option><option value="down">Down</option></select></label></>}</>}
      ...
      <h3>Equipment tray</h3><label>Existing equipment<select value={s.equipmentId} onChange={e=>s.setEquipmentId(e.target.value)}><option value="">Choose equipment</option>{s.assets.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></label><button disabled={!s.equipmentId} onClick={()=>s.selectTool('place-equipment')}>Place on map</button>
    ...
    {tab==='Layers'&&<section className="studio-tab-body" aria-label="Studio layers"><h3>Drawing layers</h3>{(Object.keys(layerNames) as StudioLayer[]).map(layer=>{const value=layerState(d.mapConfig,layer);return <div className="studio-layer-row" key={layer}><b>{layerNames[layer]}</b><label><input type="checkbox" aria-label={`Show ${layerNames[layer]}`} checked={value.visible} onChange={e=>patch({layers:{...studio.layers,[layer]:{...value,visible:e.target.checked}}},'Changed layer visibility.')}/>Show</label><label><input type="checkbox" aria-label={`Lock ${layerNames[layer]}`} checked={value.locked} onChange={e=>{patch({layers:{...studio.layers,[layer]:{...value,locked:e.target.checked}}},'Changed layer lock.');s.setSelection([]);}}/>Lock</label></div>})}
      ...
      <label>Snap spacing<select value={studio.snap??.5} onChange={e=>patch({snap:Number(e.target.value)},'Changed snapping.')}><option value="0">Off</option><option value="0.25">0.25 map units</option><option value="0.5">0.5 map units</option><option value="1">1 map unit</option><option value="2">2 map units</option></select></label>
