...
describe('co-packing records', () => {
  ...
  it('seeds reported process stages, separate case support and alternatives without physical machines or production history', () => {
    ...
    expect(pkg.assets).toHaveLength(6); expect(c.runs).toEqual([]); expect(c.products).toEqual([]); expect(c.recipes).toEqual([]);
  ...
  it('upgrades once, preserves line edits and round trips records through existing IndexedDB archives', async () => {
    ...
    upgraded=await ensurePlantSeed(seed); expect(upgraded.facility.production!.copacking!.stages).toHaveLength(36); expect(upgraded.assets).toHaveLength(6);
    ...
    await importPlantArchive(archive,'merge',pkg.facility.id); expect((await loadPlant(pkg.facility.id))!.assets).toHaveLength(6);
