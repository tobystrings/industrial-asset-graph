...
describe('review patch and plant packs', () => {
  ...
  it('is called from the live Log, empty-area, and cabinet surfaces', () => {
    ...
    const theater = readFileSync(resolve(root, 'FilmTheater.tsx'), 'utf8');
    ...
    expect(cabinet).toContain('Film');
    expect(cabinet).not.toContain('Open 3D');
    expect(theater).not.toContain('<iframe');
    expect(theater).not.toContain('aria-modal');
    expect(theater).toContain('genieFirstShow');
    expect(theater).toContain('opened');
    expect(theater).toContain('data-testid="film-mini"');
    expect(theater).toContain('data-testid="film-tour-continue"');
    expect(theater).toContain('genieMotionReduced');
    ...
    expect(theater).not.toContain('aria-modal');
    expect(theater).toContain('FILM_AUDIO_SRC');
    expect(theater).toContain('data-testid="film-clock"');
    expect(theater).toContain('data-testid="film-clock-progress"');
    expect(theater).toContain('formatFilmClock');
    expect(css).toContain('.cabinet-back-label { display: none; }');
    expect(app).toContain('filmPlaying');
    expect(app).toContain('onToggleFilm');
    expect(app).toContain('onPlayingChange={setFilmPlaying}');
    expect(dashboard).toContain('onToggleFilm');
