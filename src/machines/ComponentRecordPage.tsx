...
export default function ComponentRecordPage() {
  ...
  const photoView = imageViews(plant,asset.id).find(v => v.regions.some(r => r.entityId === component.id));
  ...
  return <main className="component-record">
    ...
    <h2>{component.label}</h2>{photoView&&<PageLink page="cabinet" details={{asset:asset.id,component:component.id,image:p.get('image')||photoView.id}}>Locate in image</PageLink>}<p className="model-record-id">{component.id}</p>
    ...
    <ConnectionLinks entityId={component.id}/>
