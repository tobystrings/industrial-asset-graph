...
export function withClimax(original:FacilityPackage):FacilityPackage {
 ...
 if(!pkg.documents.some(d=>d.id===docId))pkg.documents.push({id:docId,assetId:id,category:'Controls',title:'Climax source-backed control associations — draft',path:'docs/machines/LIEB-L2-CLIMAX-6759/source-review.md',state:'DRAFT',required:true,verificationStatus:'INFERRED',evidenceIds:[sourceId],register:{kind:'Saved control logic associations (not wiring)',entries}});
 return pkg;
}
